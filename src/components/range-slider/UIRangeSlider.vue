<template>
    <label class="mt-4">
        ...
        <teleport to="body">
            <span class="floating-label-container absolute" style="max-width: 50px" :style="position">
                <span class="range-value opacity-100 transition-opacity px-2 py-1
    ...
    setup(props) {
        ...
        watch(
            ...
            (val: number) => {
                ...
                // subtract the 7/4 of $range-handle-size
                position.value.top = `calc(${rect.y}px - 37.5px)`;
                // offset to the start of the input + half of the $range-handle-size
                position.value.left = `calc(${val}% + ${rect.left}px + 12.5px - ${25 / 100 * val}px)`;
            },
            { immediate: true }
        );
